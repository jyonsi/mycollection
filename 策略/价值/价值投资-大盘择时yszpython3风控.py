...
def initialize(context):
    ...
    g.init_value = context.portfolio.available_cash #初始资金
    # 股票池
    # 中小板 "399101.XSHE"
    ...
    #止盈止损
    run_weekly(stop_loss,3,time='open', reference_security='000300.XSHG')
    #run_daily(control_risk,3,time='open', reference_security='000300.XSHG')
    g.count_month = 0
...
def adjust_position(context, buy_stocks):
    ...
    if check_for_benchmark(context):
        ...
        buy_lists = check_for_risk(context,buy_stocks) # 获取风险权重
        if len(buy_lists) > 0:
            #分配资金
            # cash = context.portfolio.available_cash / (len(buy_lists))
            # 进行买入操作
            print("buylist是%s"%buy_lists)
            for stock in buy_lists.index:
                position = buy_lists[stock] * context.portfolio.total_value
                ...
                if not judge_More_average(
                            stock):
                    if not e_5 < -0.1 and stock in buy_lists.index:
                        result = order_value(stock, delta)
                        if not result == None:
                            log.info("买入：%s %s" %
...
def check_for_risk(context,buy_stocks):
    return g.risk_control.check_for_risk(context,buy_stocks)
...
class RiskControl(object):
    ...
    def volatility_weight(self, stocks, days):
        # 历史数据
    ...
    def check_for_risk(self, context,buy_stocks):
        weight_tmp = self.volatility_weight(buy_stocks,241)[:g.buy_stock_count]
        # 风险控制-- 趋势
        inv_stock = True
        h = history(61, '1d', 'close', [g.base], df=True)
        p = h[g.base]
        if p[-1] < p.mean():
        ...
        # 调整投资组合
        weight = pd.Series()
        for stock in weight_tmp.index:
            if inv_stock or stock in context.portfolio.positions:
            # if context.portfolio.positions:
                weight[stock] = inv_stock_ratio * weight_tmp[stock]
        print('权重之和%s'%str(weight.sum()))
