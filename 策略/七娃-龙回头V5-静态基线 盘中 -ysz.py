...
def initialize(context):
    ...
    ### 股票相关设定 ###
    # 股票类每笔交易时的手续费是：买入时佣金万分之三，卖出时佣金万分之三加千分之一印花税, 每笔交易佣金最低扣5块钱K
...
def market_open(context):
    ...
    g.df_sell=None
    #1,读取brain文件中当天的买卖信号
    df_signal = pd.read_csv(BytesIO(read_file('./data/Crouch_brain.csv')))
    ...
    for stockcode in context.portfolio.positions:
        ...
        if ret > 2:
            if order_target(stockcode,0) != None:
                write_file('./data/Crouch_log.csv', str('%s,开盘止盈,%s,周期:%s,盈利:%s\n' % (context.current_dt.time(),stockcode,duration,ret)),append = True)
                continue
            
        if stockcode in g.df_sell['code'].values:
            if order_target(stockcode,0) != None:
                write_file('./data/Crouch_log.csv', str('%s,开盘清仓,%s,周期:%s,盈利:%s\n' % (context.current_dt.time(),stockcode,duration,ret)),append = True)
                continue
    ...
    for stockcode in context.portfolio.positions:
        ...
        if (current_data[stockcode].last_price < 1.02*current_data[stockcode].low_limit):
            ...
            if (df_ticks['volume'].values[0] > df_price['volume'].values[0]) or (df_ticks['volume'].values[0] > df_price['volume'].values[1]):
                if order_target(stockcode,0) != None:
                    write_file('./data/Crouch_log.csv', str('%s,盘中近停,%s,周期:%s,盈利:%s\n' % (context.current_dt.time(),stockcode,duration,ret)),append = True)
                    continue
        if ret < -0.1:
            if order_target(stockcode,0) != None:
                write_file('./data/Crouch_log.csv', str('%s,盘中止损,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
                continue
        elif ret > 2:
            if order_target(stockcode,0) != None:
                write_file('./data/Crouch_log.csv', str('%s,盘中止盈,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
                continue
...
def market_close(context):
    ...
    for stockcode in context.portfolio.positions:
        ...
        if (current_data[stockcode].last_price < 1.02*current_data[stockcode].low_limit):
            ...
            if (df_ticks['volume'].values[0] > df_price['volume'].values[0]) or (df_ticks['volume'].values[0] > df_price['volume'].values[1]):
                if order_target(stockcode,0) != None:
                    write_file('./data/Crouch_log.csv', str('%s,尾盘近停,%s,周期:%s,盈利:%s\n' % (context.current_dt.time(),stockcode,duration,ret)),append = True)
                    continue
        if ret < -0.1:
            if order_target(stockcode,0) != None:
                write_file('./data/Crouch_log.csv', str('%s,尾盘止损,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
                continue
        elif ret > 2:
            if order_target(stockcode,0) != None:
                write_file('./data/Crouch_log.csv', str('%s,尾盘止盈,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
                continue
        
        if duration >=3 and ret <(-0.05+0.015*(duration-3))/g.roundratio:
            if order_target(stockcode,0) != None:
                write_file('./data/Crouch_log.csv', str('%s,尾盘短清,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
                continue
            
        if duration >=6 and g.roundratio*rise_ratio <(0.004*(2**(duration//4-1))):
            if order_target(stockcode,0) != None:
                write_file('./data/Crouch_log.csv', str('%s,尾盘失速,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
                continue        
    ##接着买的操作
    if context.portfolio.available_cash <10000: #钱不够了
        return
    ...
    for i in range(len(g.df_buy)):
        ...
        if current_data[stockcode].paused != True:
            if order_target_value(stockcode,cash_perstk) != None:
                write_file('./Crouch_log/Crouch_log.csv', str('%s,尾盘买入,%s,%s\n' % (context.current_dt.time(),stockcode,current_data[stockcode].last_price)),append = True)
...
def after_close_eye(context):
    ...
    df_existing = pd.read_csv(BytesIO(read_file('./data/Crouch_eye.csv')))
...
def after_close_brain(context):
    ...
    all_data = get_current_data()
    ##选股1，读取eye文件30天内的信号
    df_waiting = pd.read_csv(BytesIO(read_file('./data/Crouch_eye.csv')))
    ...
    df_existing = pd.read_csv(BytesIO(read_file('./data/Crouch_brain.csv')))
    df_existing['date'] = pd.to_datetime(df_existing['date']).dt.date
        ...
        if dur_MinD ==1 and dur_TD <20:
            if riselevel >1.1 and riselevel <1.8:
                if price_D >5 and price_D <40:
                    flag = 'buy_BL'
                    write_file('./data/Crouch_brain.csv', str('%s,%s,%s,%s,%s\n' % (today_date,flag,stockcode,pop,price_D)),append = True)
                    write_file('./data/Crouch_log.csv', str('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % (T1_date,maxd_date,mind_date,today_date,flag,stockcode,
    ...
    for stockcode in context.portfolio.positions:
        ...
        if ret < -0.1:
            write_file('./data/Crouch_brain.csv',str('%s,sell,%s,ZS,%s\n' % (today_date,stockcode,price)),append = True) #记录在任务清单中，方便明日盘前载入四大list执行
            write_file('./data/Crouch_log.csv', str('%s,止损信号,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
            continue
        elif ret > 2:
            write_file('./data/Crouch_brain.csv',str('%s,sell,%s,CY,%s\n' % (today_date,stockcode,price)),append = True) #记录在任务清单中，方便明日盘前载入四大list执行
            write_file('./data/Crouch_log.csv', str('%s,超盈信号,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
            continue
        
        if duration >=3 and ret <(-0.05+0.015*(duration-3))/g.roundratio:
            write_file('./data/Crouch_brain.csv',str('%s,sell,%s,DQ,%s\n' % (today_date,stockcode,price)),append = True) #记录在任务清单中，方便明日盘前载入四大list执行
            write_file('./data/Crouch_log.csv', str('%s,短期信号,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
            continue
        
        if duration >=6 and g.roundratio*rise_ratio <(0.004*(2**(duration//4-1))):
            write_file('./data/Crouch_brain.csv',str('%s,sell,%s,SS,%s\n' % (today_date,stockcode,price)),append = True) #记录在任务清单中，方便明日盘前载入四大list执行
            write_file('./data/Crouch_log.csv', str('%s,失速信号,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
            continue
        ...
        if df_price['close'].values[-1] <= df_price['low_limit'].values[-1]:
            ...
            if (df_price['volume'].values[-1] > df_price['volume'].values[-2]) or (df_price['volume'].values[-1] > df_price['volume'].values[-3]):
                write_file('./data/Crouch_brain.csv',str('%s,sell,%s,DT,%s\n' % (today_date,stockcode,price)),append = True) #记录在任务清单中，方便明日盘前载入四大list执行
                write_file('./data/Crouch_log.csv', str('%s,跌停信号,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
                continue
        ...
        if (df_value['turnover_ratio'].values[0] > 2*df_value['turnover_ratio'].values[1]) and (df_value['turnover_ratio'].values[0] > 0.99*to_30_max):
            if (df_price['close'].values[-1] >= df_price['high_limit'].values[-1]) or (df_value['turnover_ratio'].values[0] <6) :   #天量中若当日涨停略过,换手绝对值低于6%也可略过
                continue
            write_file('./data/Crouch_brain.csv',str('%s,sell,%s,TL,%s\n' % (today_date,stockcode,price)),append = True) #记录在任务清单中，方便明日盘前载入四大list执行
            write_file('./data/Crouch_log.csv', str('%s,天量信号,%s,周期:%s,盈利:%s\n' % (today_date,stockcode,duration,ret)),append = True)
            continue
