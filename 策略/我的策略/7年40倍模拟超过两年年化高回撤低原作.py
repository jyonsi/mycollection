...
def get_bull_bear_signal_minute(context): #牛熊判断
    ...
    maslope20, maangle20 = get_trend(g.MA[0], 10, '1d', curr_date, 20, 'ma')
    maslope60, maangle60 = get_trend(g.MA[0], 10, '1d', curr_date, 60, 'ma')
    #maslope120, maangle120 = get_trend('000300.XSHG', 10, '1d','ma',number=120)

    emaslope20, emaangle20 = get_trend(g.MA[0], 10, '1d', curr_date, 20, 'ema')
    emaslope60, emaangle60 = get_trend(g.MA[0], 10, '1d', curr_date, 60, 'ema')
    #emaslope120, emaangle120 = get_trend('000300.XSHG', 10, '1d','ema',number=120)

    if g.isbull:
        if (maangle20 <10 and emaangle20 <15) and (maangle60 <5 and emaangle60 <10):
            g.isbull = False
    else:
        if (maangle20 >10 and emaangle20 >15) and (maangle60 >5 and emaangle60 >10):
            g.isbull = True
