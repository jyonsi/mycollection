...
def get_stock_list(context):
    ...
    # 过滤行业白名单以外的股票
    stock_list = [ stockcode for stockcode in dict_indus.keys() if dict_indus[stockcode]['sw_l1']['industry_code'] in g.indus_list]
...
def get_bull_bear_signal_minute(context): #牛熊判断
    ...
    # maslope10, maangle10 = get_trend(g.MA[0], g.MA[1], '1d', curr_date, 10, 'ma')
    # maslope20, maangle20 = get_trend(g.MA[0], g.MA[2], '1d', curr_date, 20, 'ma')
    # emaslope10, emaangle10 = get_trend(g.MA[0], g.MA[1], '1d', curr_date, 10, 'ema')
    # emaslope20, emaangle20 = get_trend(g.MA[0], g.MA[2], '1d', curr_date, 20, 'ema')
    
    # baseratio = round(math.degrees(math.atan(g.threshold)),2)

    # if g.is_bull:
    #     if (maangle10 <baseratio and emaangle10 <baseratio) and (maangle20 <baseratio and emaangle20 <baseratio):
    #         g.is_bull = False
    # else:
    #     if (maangle10 > baseratio and emaangle10 >baseratio) and (maangle20 >baseratio and emaangle20 >baseratio):
    #         g.is_bull = True
...
def get_stock_rank_m_m():
    ...
    dict_indes = [ get_industry(stock, date=None) for stock in stock_list ]
