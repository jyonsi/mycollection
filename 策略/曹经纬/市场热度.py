# 克隆自聚宽文章：https://www.joinquant.com/post/31706
# 标题：2020-2021年年化180%的高收益策略
# 作者：曹经纬
...
def check_stocks_initialize():
    ...
    # 行业列表
    #g.industry_list = ["801030","801080","801110","801120","801150","801730","801880","801890"]
...
def financial_statements_filter(context, security_list):
    ...
    security_list = financial_data_filter_qujian(security_list, indicator.roe, (5,35))
    security_list = financial_data_filter_qujian(security_list, indicator.gross_profit_margin, (20,100))
    security_list = financial_data_filter_qujian(security_list, valuation.market_cap, (10,1000000))
    ### _财务指标筛选函数-结束 ###
...
##################################  交易函数群 ##################################
# 买入    
def buy(context, buy_list):
    for stock in buy_list:
        order_target_value(stock, context.portfolio.total_value / g.max_hold_stocknum, MarketOrderStyle())    
        
# 卖出        
def sell(context, buy_list):
    sell_list = [security for security in context.portfolio.positions.keys() if security not in buy_list]
...
def check_for_benchmark(security):
    return RSI_judge_qujian('000300.XSHG', (44,99), 60)
